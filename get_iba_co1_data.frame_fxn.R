...
source("spikes_controls_fxns_data.frame.R")
...
get_iba_co1_data <- function(data_path,
    ...
    if (country=="SE") {
        ...
        counts <- read.delim(paste0(data_path,"cleaned_noise_filtered_cluster_counts_SE.tsv"),header=TRUE,sep="\t")
    ...
    if (country=="MG") {
        ...
        # Get data files
        cat("Reading data files, this may take a while...\n")
        counts <- read.delim(paste0(data_path,"cleaned_noise_filtered_cluster_counts_MG.tsv"),header=TRUE,sep="\t")
